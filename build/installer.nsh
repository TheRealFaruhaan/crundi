...
!macro customInstall
  ...
  ; alive when the physical screen is locked (RPA trick via tscon.exe).
  ; The task runs disconnect-rdp.bat with highest privileges; the bot triggers
  ; it at runtime via "schtasks /run" without needing UAC.
