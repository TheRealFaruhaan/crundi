...
const TOOLS = [
  ...
  { name: 'mindmap_add_node', description: 'Add a mindmap idea node. Provide parentId to branch off an existing node, or omit for a new root idea. To extend a Kanban task pass taskId; to link to a specific subtask also pass todoId (project defaults to the current project).', inputSchema: { type: 'object', properties: { text: { type: 'string', description: 'Node text' }, parentId: { type: 'string', description: 'Parent node id (omit for a root idea)' }, note: { type: 'string', description: 'Optional longer note' }, taskId: { type: 'string', description: 'Optional Kanban task id to link this node to' }, todoId: { type: 'string', description: 'Optional todo (subtask) id within the task to link to' }, project: { type: 'string', description: 'Project alias for the linked task (defaults to current project)' } }, required: ['text'] } },
