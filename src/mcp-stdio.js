...
const TOOLS = [
  ...
  // ─── Mindmap tools (global brainstorming map) ───
  { name: 'mindmap_list', description: 'List all mindmap nodes (global). Each node has id, text, note, parentId (null=root), and `linkedTaskInfo` — if the node links to a Kanban task, this holds that task\'s live project, title, description, status, deleted flag, and todo progress (todosDone/todosTotal); if the link is dangling it is marked missing. Use this to brainstorm tasks into detail or capture free-standing ideas.', inputSchema: { type: 'object', properties: {} } },
  { name: 'mindmap_get_subtree', description: 'Get one node and ALL of its descendants as a nested tree (children → grandchildren → …), each with its linked-task info. Use this instead of mindmap_list to fetch a single branch and save tokens.', inputSchema: { type: 'object', properties: { id: { type: 'string', description: 'Node id to start from' } }, required: ['id'] } },
  { name: 'mindmap_get_children', description: 'Get only the DIRECT children of a node (one level, no descendants); omit id to get the root nodes. Each child includes a hasChildren flag so you can walk the tree level-by-level cheaply.', inputSchema: { type: 'object', properties: { id: { type: 'string', description: 'Parent node id; omit/empty for root nodes' } } } },
  { name: 'mindmap_add_node', description: 'Add a mindmap idea node. Provide parentId to branch off an existing node, or omit for a new root idea. To extend a Kanban task, pass taskId (project defaults to the current project) so the node links to that task.', inputSchema: { type: 'object', properties: { text: { type: 'string', description: 'Node text' }, parentId: { type: 'string', description: 'Parent node id (omit for a root idea)' }, note: { type: 'string', description: 'Optional longer note' }, taskId: { type: 'string', description: 'Optional Kanban task id to link this node to' }, project: { type: 'string', description: 'Project alias for the linked task (defaults to current project)' } }, required: ['text'] } },
  ...
  { name: 'mindmap_link_node', description: 'Link a mindmap node to a Kanban task.', inputSchema: { type: 'object', properties: { id: { type: 'string' }, taskId: { type: 'string' }, project: { type: 'string', description: 'Project alias (defaults to current project)' } }, required: ['id', 'taskId'] } },
...
const ALIAS_TOOLS = new Set(['browser_open', 'browser_list', 'register_service', 'spawn_terminal', 'list_terminals', 'terminal_input', 'terminal_output', 'terminal_wait', 'close_terminal',
  ...
  'mindmap_add_node', 'mindmap_link_node',
  'secret_get']);
