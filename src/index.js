...
// Launch-time check (non-elevating): report whether locked-screen capture is
// set up. We never auto-trigger UAC here — the installer creates it, and the
// desktop app offers a one-click elevated "Set up" in the wizard + a launch
// prompt. This is just a visible heads-up in the logs.
if (process.platform === 'win32') {
  import('./rpa.js')
    .then(rpa => {
      if (rpa.isTaskInstalled()) console.log('[crundi] Locked-screen capture: DisconnectRDP task is configured.');
      else console.warn('[crundi] Locked-screen capture: DisconnectRDP task NOT configured — set it up from the app (Settings/setup) to capture while the screen is locked.');
    })
    .catch(() => { /* non-fatal */ });
}
